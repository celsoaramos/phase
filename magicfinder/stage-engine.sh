...
( cd client && pnpm install --frozen-lockfile && \
  ENGINE_WASM_URL="${BASE}/${WASM_NAME}" CARD_DATA_URL="${BASE}/${CARDS_NAME}" DATA_BASE_URL="https://data.phase-rs.dev" pnpm build )
...
grep -qF "${BASE}/${CARDS_NAME}" "${OUT}/engine-worker.js" || { echo "::error::worker não referencia o card-data do prefixo"; exit 1; }
